...
class Discriminator(nn.Module):
    ...
    def __preprocess_neighbors_sumavepool(self, batch_graph):
        ...
        for i, graph in enumerate(batch_graph):
            start_idx.append(start_idx[i] + len(graph.g))
        Adj_block = torch.zeros(start_idx[i+1],start_idx[i+1])
        for i, graph in enumerate(batch_graph): 
            Adj_block[start_idx[i]:start_idx[i+1],start_idx[i]:start_idx[i+1]] = graph.edge_mat
        #Adj_block = torch.div(torch.add(Adj_block_elem, Adj_block_elem.transpose(0, 1)), 2.0)

        for x_i in range(start_idx[i+1]):
            Adj_block[x_i,x_i] = 1
