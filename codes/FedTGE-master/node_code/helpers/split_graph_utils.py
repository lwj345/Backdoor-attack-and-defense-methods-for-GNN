...
def split_Louvain(args, data):
    """
    Splits the data using Louvain community detection and distributes nodes across clients.
    :param args: Arguments containing parameters like overlapping_rate, drop_edge, etc.
    :param data: Input graph data.
    :return: List of subgraphs for each client.
    """
    args.delta = 40
    ...
    tmp_cluster2node = {}
    max_iterations = 20  # 设置最大循环次数为 20

    for cluster in cluster2node:
        iteration_count = 0  # 初始化循环计数器
        while len(cluster2node[cluster]) > max_len:
            print(f"Cluster {cluster}, Iteration {iteration_count + 1}, Current size: {len(cluster2node[cluster])}")
            ...
            if iteration_count > max_iterations:
                print(f"Warning: Exceeded max iterations for cluster {cluster}")
                break  # 跳出循环以防止无限循环
    ...
    client_node_idx = {idx: [] for idx in range(client_num)}
    idx = 0
    max_iterations_while = 20  # 设置最大循环次数为 20
    iteration_count_while = 0

    for (cluster, node_list) in orderedc2n:
        iteration_count_while = 0  # 初始化循环计数器
        while len(node_list) + len(client_node_idx[idx]) > max_len_client + delta:
            print(
                f"Current node_list length: {len(node_list)}, client_node_idx[{idx}] length: {len(client_node_idx[idx])}")
            iteration_count_while += 1
            if iteration_count_while > max_iterations_while:
                print(f"Warning: Exceeded max iterations in assigning cluster {cluster} to client {idx}")
                break  # 跳出循环以防止无限循环
