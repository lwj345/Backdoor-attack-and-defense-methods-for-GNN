...
class Evaluator:
    ...
    def test_gat(self, nlayers, model_type, verbose=False):
        ...
        model.fit(feat_syn, adj_syn, labels_syn, np.arange(len(feat_syn)), noval=noval, data=data,
                    # train_iters=2, normalize=True, verbose=verbose)
        ...
        if args.backdoor:
            ...
            trigger_generator = self.get_trojan_networks()
            ...
            if args.dataset in ['flickr', 'reddit']:
                idx_test_attack = hs.randomly_obtain_attach_nodes_induct(self.args.seed, idx_test_attack, int(1.0*idx_test_attack.shape[0]))
            ...
            if verbose:
                print("Poisoned Test set results (ASR):",
                    ...
                    "accuracy= {:.4f}".format(poisoned_acc_test.item()))
        return res

    def get_trojan_networks(self):
        device = self.device
        # if self.args.pure:
        #     return None
        if self.args.best_epoch == 0:
            trigger_generator = torch.load(f'saved_ours/trojan_networks/trigger-gen_{self.args.dataset}_{self.args.reduction_rate}_{self.args.seed}.pt')
        else:
            trigger_generator = torch.load(f'saved_ours/trojan_networks/trigger-gen_{self.args.dataset}_{self.args.reduction_rate}_{self.args.seed}-{self.args.best_epoch}.pt')
        trigger_generator.to_device(device)
        # trigger_generator.trojan.to(device)
    ...
    def get_syn_data(self, model_type=None):
        ...
        args = self.args
        if self.args.pure:
            adj_syn = torch.load(f'saved_ours/adj_{args.dataset}_{args.reduction_rate}_{args.seed}-pure.pt', map_location='cuda')#
            feat_syn = torch.load(f'saved_ours/feat_{args.dataset}_{args.reduction_rate}_{args.seed}-pure.pt', map_location='cuda')#-pure
        else:
            if self.args.best_epoch == 0:
                adj_syn = torch.load(f'saved_ours/adj_{args.dataset}_{args.reduction_rate}_{args.seed}.pt', map_location='cuda')#-pure
                feat_syn = torch.load(f'saved_ours/feat_{args.dataset}_{args.reduction_rate}_{args.seed}.pt', map_location='cuda')#-pure
            else:
                adj_syn = torch.load(f'saved_ours/adj_{args.dataset}_{args.reduction_rate}_{args.seed}-{self.args.best_epoch}.pt', map_location='cuda')#-pure
                feat_syn = torch.load(f'saved_ours/feat_{args.dataset}_{args.reduction_rate}_{args.seed}-{self.args.best_epoch}.pt', map_location='cuda')#-pure
    ...
    def test(self, nlayers, model_type, verbose=True):
        ...
        model.fit_with_val(feat_syn, adj_syn, labels_syn, data,
                    ...
                    #  train_iters=2, normalize=True, verbose=True, noval=noval)
    ...
    def train(self, verbose=True):
        ...
        for model_type in ['GCN',  'GraphSage', 'SGC1', 'MLP', 'APPNP1', 'Cheby']:
            ...
            res = np.array(res)
            # print('CTA/ASR Mean Accuracy:',
            print('CTA/Train/ASR Mean Accuracy:',
