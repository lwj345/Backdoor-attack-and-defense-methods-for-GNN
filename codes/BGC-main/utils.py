...
class Transd2Ind:
    ...
    def retrieve_class(self, c, num=256):
        ...
        idx = idx[self.labels_train == c]#idx[self.class_dict['class_%s'%c]]
        return np.random.permutation(idx)[:num]

    def retrieve_class_backdoor(self, c, poisoned_labels, num=256):
        poisoned_labels = poisoned_labels.cpu().numpy()
        if self.class_dict is None:
            self.class_dict = {}
            for i in range(self.nclass):
                self.class_dict['class_%s'%i] = (poisoned_labels == i)
        idx = np.arange(len(poisoned_labels))
        idx = idx[self.class_dict['class_%s'%c]]
        return np.random.permutation(idx)[:num]

    def retrieve_class_sampler_backdoor(self, c, adj, poisoned_labels, idx_poisoned, transductive, num=256, args=None):
        poisoned_labels = poisoned_labels.cpu().numpy()
        if self.class_dict2 is None:
            self.class_dict2 = {}
            for i in range(self.nclass):
                if transductive:
                    idx = idx_poisoned[poisoned_labels == i]
                else:
                    idx = np.arange(len(poisoned_labels))[poisoned_labels==i]
    ...
    def retrieve_class_sampler_clean(self, c, adj, transductive, num=256, args=None):
        # if self.class_dict2 is None:
        class_dict2 = {}
        for i in range(self.nclass):
            if transductive:
        ...
        if self.samplers is None:
            self.samplers = []
            for i in range(self.nclass):
                node_idx = torch.LongTensor(class_dict2[i])
                self.samplers.append(NeighborSampler(adj,
                                    node_idx=node_idx,
                                    sizes=sizes, batch_size=num,
                                    num_workers=12, return_e_id=False,
                                    num_nodes=adj.size(0),
                                    shuffle=True))
        batch = np.random.permutation(class_dict2[c])[:num]
        out = self.samplers[c].sample(batch)
        return out
